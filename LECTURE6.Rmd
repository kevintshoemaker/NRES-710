...
You can have multiple explanatory variables (multiple linear regression). But we will focus on *simple linear regression* here. 
...
Now that we have data, let's run a linear regression analysis!
...
Let's quickly review what residuals are [quick whiteboard demo]
...
Variance Inflation Factors (VIFs) are a diagnostic tool to help you evaluate if you have a multicollinearity problem. In general VIFs less than 4 are considered okay. On the other hand, VIFs greater than 5 or 10 indicate serious problems!
