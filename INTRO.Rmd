...
1.	Categorical (Can be quantitative or qualitative) – represents qualitative characteristics      
    a.	Nominal (labels, > 2 categories)     
    b.	Dichotomous (2 categories)    
    c.	Ordinal (ordered categories: category 3 is bigger than 2 is bigger than 1)  
...
2.	Quantitative     
    ...
    b.	Continuous: not countable, but can be measured (total body length)
        ...
        ii.	Ratio: zero means complete absence of a thing (e.g., temperature in Kelvin). If zero means absence, then the ratio between two values is meaningful (something that weighs 4 g has twice the mass of something that weighs 2 g). Note that you can't say that for interval data (e.g., something with a temperature of 4 C is not twice as hot as something at 2 C).    
...
Use variable names that will not confuse you (but variable names can't start with a number). 
