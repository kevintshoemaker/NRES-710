...
The figure below illustrates the basic concepts of statistics. The bold line separates the unseen population of interest (left) from the observed sample (right):     
...
Specifically, we use a special type of probability distribution known as a "sampling distribution" to quantify how much noise to expect from our random sampling process alone. 

In this class, we define the probability of any specific outcome as the expected frequency of that outcome.
...
The distribution of possible outcomes from your sampling process (assuming a fair coin) is an example of a *sampling distribution*.

The notion that the coin is a fair coin is an example of a *null hypothesis*... 

## Null hypotheses and statistical tests

The null hypothesis is the notion that the signal you are looking for is entirely absent in the population of interest. 

Under the null hypothesis, any hypothetical random samples you collect might have some apparent signal, but that apparent signal is just an artifact of random sampling, and not representative of the population (after all, the signal is completely absent in the population). 
...
The lower the p-value, the more surprised you will be that your observed data could simply be a result of random chance alone AND the more likely you might be to *reject* the null hypothesis! 
...
A result 'at least as extreme as your observed result' means a result that has at least as much "signal" as your observed data. 
...
**Q** Would you be comfortable rejecting the null hypothesis if random sampling under the null hypothesis yielded a result as or more extreme approximately 20% of the time (p = 0.2)? 
...
Fisher suggested we give her 8 seemingly identical cups of tea at once, 4 with milk first and 4 with tea first. The woman is asked to identify the four cups that were poured with milk first. 
