...
1. RR: PinyonRoadrunner density (birds/km2). This is your response variable
...
6. PatchSize: Area (ha) of the woodland patch where observations were made (or the closest patch to the observation point), estimated from aerial photography
...
Be aware that this is a simulated data set for a fictitious species (yes, I have been pulling your leg up to this point). Actually, the true habitat relationships for this species are perfectly known as the parameters in a linear equation used to calculate the abundances.

You should therefore be able to reconstruct the true parameters through a multiple linear equation analysis! However, some challenges that may make this a nontrivial exercise in regression modeling:

a. Some random noise (uncorrelated error) has been added to the calculation
...
e. There may be interaction effects in therelationship between predictor and response variables
...
* You can use the `update()` function to quickly add or remove parameters from existing regression (lm) models
* Use regression diagnostic plots
* Use the `poly` function to add polynomial terms to a regression model. 
* Try using the `effects` package to easily display (graphically) effects of predictor variables modeled from your regression models, including interaction and polynomial terms
...
* Use the `anova()` function on multiple, nested models to statistically test model parsimony using an 'Extra Sum of Squares' F-Test (increase in variance explained vs. number of parameters added). You could also use the AIC function to calculate AIC coefficients (which we will cover later in the course). 
